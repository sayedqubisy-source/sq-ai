...
setInterval(()=>{if(!db)return;try{const cutoff=new Date(Date.now()-20*60*1000).toISOString();for(const j of db.prepare("SELECT id FROM video_jobs WHERE status='running' AND started_at IS NOT NULL AND started_at < ?").all(cutoff))finalizeFailure(j.id,'video_job_timeout_after_restart');}catch(error){console.error('SQ AI video recovery failed:',error?.message||error);}},2*60*1000).unref();

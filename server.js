...
function loginGuard(email) {
  ...
  if (record.resetAt <= Date.now()) { loginFailures.delete(email); return false; }
  return record.count >= 8;
}
function recordLoginFailure(email) {
  const now = Date.now();
  const record = loginFailures.get(email);
  if (!record || record.resetAt <= now) loginFailures.set(email, { count:1, resetAt:now + 15*60*1000 });
  else record.count += 1;
}
function clearLoginFailures(email) { loginFailures.delete(email); }
setInterval(() => {
  const now = Date.now();
  for (const [email, record] of loginFailures) if (record.resetAt <= now) loginFailures.delete(email);
...
function modelList(primary) { return [...new Set([primary, 'openrouter/free'].filter(Boolean))]; }
function shouldRetry(status) { return status===408 || status===409 || status===425 || status===429 || status>=500; }
function extractTextContent(value) {
  if (typeof value === 'string') return value.trim();
  if (Array.isArray(value)) return value.map(part => typeof part === 'string' ? part : part?.text || part?.content || part?.value || '').filter(Boolean).join('\n').trim();
  return '';
}
  ...
  for (const candidate of modelList(model || process.env.OPENROUTER_MODEL)) {
    for (let attempt=0; attempt<2; attempt++) {
      try {
        ...
        if (r.ok) {
          const text = extractTextContent(data?.choices?.[0]?.message?.content) || extractTextContent(data?.choices?.[0]?.text) || extractTextContent(data?.output_text);
          if (text) return text;
...
async function openRouterImage(prompt, model) {
  const key=process.env.OPENROUTER_API_KEY;
  if(!key) throw Object.assign(new Error('openrouter_not_configured'),{code:'openrouter_not_configured',status:502});
  const selectedModel=model || process.env.OPENROUTER_IMAGE_MODEL || 'google/gemini-3.1-flash-image';
  let lastError;
  for(let attempt=0;attempt<2;attempt++){
    try{
      const r=await fetchJsonWithTimeout('https://openrouter.ai/api/v1/images',{
        method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json','HTTP-Referer':process.env.APP_URL || 'http://localhost:3000','X-Title':'SQ AI'},
...
async function generateWithHuggingFace({prompt}){
  ...
  try{
    ...
    return {provider:'huggingface',model,video_url:await saveVideoBlob(blob)};
  }catch(e){
...
async function secureVideoGenerate({tool,prompt,language,platform}){
  ...
  const url=process.env.VIDEO_API_URL,key=process.env.VIDEO_API_KEY;
  if(!url || !key)throw Object.assign(new Error('video_provider_not_configured'),{code:'video_provider_not_configured',status:502});
  let endpoint;try{endpoint=new URL(url);}catch{throw Object.assign(new Error('video_provider_url_invalid'),{code:'video_provider_url_invalid',status:500});}
  if(!['https:','http:'].includes(endpoint.protocol))throw Object.assign(new Error('video_provider_url_invalid'),{code:'video_provider_url_invalid',status:500});
...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'SQ AI',version:'3.7.0',video_mode:process.env.PAID_VIDEO_ENABLED==='true'?'paid':'free'}));
...
app.post('/api/auth/signup',async(req,res)=>{
  try{
    ...
    const r=db.prepare("INSERT INTO users(email,name,password_hash,plan,credits) VALUES(?,?,?,'starter',?)").run(email,name,hash,plans.starter.credits);
    const token=makeToken();
    db.prepare("INSERT INTO sessions(user_id,token_hash,expires_at) VALUES(?,?,datetime('now','+30 days'))").run(r.lastInsertRowid,hashToken(token));
    setSessionCookie(res,token);
    return res.status(201).json({user:publicUser(getUserById(r.lastInsertRowid))});
  }catch(e){console.error('signup_failed',e?.message || e);return res.status(500).json({error:'signup_failed'});}
});
...
app.post('/api/auth/login',async(req,res)=>{
  try{
    ...
    if(!validEmail(email))return res.status(400).json({error:'valid_email_required'});
    if(loginGuard(email))return res.status(429).json({error:'too_many_login_attempts'});
    const u=db.prepare('SELECT * FROM users WHERE email=?').get(email);
    if(!u || !u.password_hash || !(await verifyPassword(password,u.password_hash))){recordLoginFailure(email);return res.status(401).json({error:'invalid_email_or_password'});}
    clearLoginFailures(email);
    const token=makeToken();
    db.prepare("INSERT INTO sessions(user_id,token_hash,expires_at) VALUES(?,?,datetime('now','+30 days'))").run(u.id,hashToken(token));
    setSessionCookie(res,token);
    return res.json({user:publicUser(getUserById(u.id))});
  }catch(e){console.error('login_failed',e?.message || e);return res.status(500).json({error:'login_failed'});}
});
...
app.post('/api/auth/set-password',auth,async(req,res,next)=>{try{const password=String(req.body.password||'');if(password.length<8)return res.status(400).json({error:'password_min_8_characters'});db.prepare('UPDATE users SET password_hash=? WHERE id=?').run(await hashPassword(password),req.user.id);res.json({ok:true});}catch(e){next(e);}});
app.patch('/api/account',auth,(req,res,next)=>{try{const name=limitText(req.body.name,100);db.prepare('UPDATE users SET name=? WHERE id=?').run(name,req.user.id);res.json({user:publicUser(getUserById(req.user.id))});}catch(e){next(e);}});
app.post('/api/signup',(req,res)=>res.status(410).json({error:'legacy_signup_disabled',message:'Use /api/auth/signup.'}));
app.get('/api/usage',auth,(req,res)=>{const u=db.prepare('SELECT credits FROM users WHERE id=?').get(req.user.id);const total=db.prepare('SELECT COALESCE(SUM(units),0) n FROM usage WHERE user_id=?').get(req.user.id).n;res.json({credits:u?.credits ?? 0,total_units:total});});
...
app.post('/api/tools/generate',auth,async(req,res,next)=>{
  try{
    ...
    if(videoTools.has(tool)){
      ...
      return res.json({result:videoUrl,video_url:videoUrl,provider:data?.provider||'custom',model:data?.model||null,free:data?.free===true,credits_remaining:fresh?.credits ?? 0});
    }
    if(imageTools.has(tool)){
      const data=await openRouterImage(`${prompt}\nPlatform: ${platform||'general'}\nLanguage/context: ${language||'English'}`,process.env.OPENROUTER_IMAGE_MODEL);
      ...
      if(!consumeCredit(req.user.id,`tool:${tool}`))return res.status(402).json({error:'credits_exhausted'});
      const fresh=db.prepare('SELECT credits FROM users WHERE id=?').get(req.user.id);
      return res.json({result:imageResult,image_url:imageResult,credits_remaining:fresh?.credits ?? 0});
    }
    ...
    if(!consumeCredit(req.user.id,`tool:${tool}`))return res.status(402).json({error:'credits_exhausted'});
    const fresh=db.prepare('SELECT credits FROM users WHERE id=?').get(req.user.id);
    return res.json({result:text,credits_remaining:fresh?.credits ?? 0});
  }catch(e){next(e);}
});
...
app.post('/api/campaigns/generate',auth,async(req,res,next)=>{
  try{
    if(req.user.credits<1)return res.status(402).json({error:'credits_exhausted'});
    const product=limitText(req.body.product,1000),audience=limitText(req.body.audience,1000),goal=limitText(req.body.goal,100),platform=limitText(req.body.platform,100),input=limitText(req.body.input||req.body.prompt,4000);
    if(!product)return res.status(400).json({error:'product_required'});
    const prompt=`Product/service: ${product}\nTarget audience: ${audience||'Not specified'}\nGoal: ${goal||'Sales'}\nPlatform: ${platform||'Multi-platform'}\nAdditional information: ${input||'None'}\n\nCreate a practical campaign including audience, offer, strategy, hooks, ad copy, creative directions, video script, CTA, budget split, KPIs and testing plan.`;
    const text=await openRouterChat([{role:'system',content:'You are SQ AI. Build a practical advertising campaign. Use clear headings and actionable recommendations.'},{role:'user',content:prompt}],process.env.OPENROUTER_MODEL);
    if(!text.trim())return res.status(502).json({error:'ai_empty_result'});
...
app.use((err,req,res,next)=>{
  ...
  console.error('api_error',{code:err?.code || 'server_error',status,path:req.path,message:err?.message || 'unknown'});
  res.status(status>=400&&status<600?status:500).json({error:err?.code || 'server_error',message:process.env.NODE_ENV==='production'?'Request failed.':err?.message || 'Request failed.'});
});

const port=Number(process.env.PORT||3000);
const server=app.listen(port,'0.0.0.0',()=>console.log(`SQ AI listening on 0.0.0.0:${port}`));
function shutdown(signal){
  console.log(`SQ AI ${signal}: shutting down`);

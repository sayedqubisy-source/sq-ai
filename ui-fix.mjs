...
const runtimeMarkers={
  ...
  music:'<script defer src="/music-tool-fix.js?v=sqai-music-20260914"></script>'
};
...
function clean(html){return html.replace(/\s*<!-- SQAI_LAZY_ENHANCEMENTS -->[\s\S]*?<\/script>/g,'').replace(/\s*<script(?: defer)? src="\/app-fixes\.js(?:\?[^\"]*)?"><\/script>/g,'').replace(/\s*<script(?: defer)? src="\/sq-ai-runtime\.js(?:\?[^\"]*)?"><\/script>/g,'').replace(/\s*<script(?: defer)? src="\/ui-theme\.js(?:\?[^\"]*)?"><\/script>/g,'').replace(/\s*<script(?: defer)? src="\/tools-runtime\.js(?:\?[^\"]*)?"><\/script>/g,'').replace(/\s*<script(?: defer)? src="\/media-studio\.js(?:\?[^\"]*)?"><\/script>/g,'').replace(/\s*<script(?: defer)? src="\/music-tool-fix\.js(?:\?[^\"]*)?"><\/script>/g,'')}
try{if(fs.existsSync(indexFile)){const original=fs.readFileSync(indexFile,'utf8');let html=clean(original);const scripts=`\n  ${runtimeMarkers.app}\n  ${runtimeMarkers.runtime}`;html=html.replace(/<\/head>/i,`${runtimeMarkers.theme}${scripts}\n</head>`);if(!html.includes(legalMarker))html=html.replace(/<\/footer>/i,`${legalLinks}\n  </footer>`);if(html!==original)fs.writeFileSync(indexFile,html,'utf8')}}catch(error){console.error('SQ AI UI fix injection failed:',error?.message||error)}
try{if(fs.existsSync(toolsFile)){const original=fs.readFileSync(toolsFile,'utf8');let html=clean(original);html=html.replace(/<\/head>/i,`${runtimeMarkers.tools}\n${runtimeMarkers.media}\n${runtimeMarkers.music}\n</head>`);if(html!==original)fs.writeFileSync(toolsFile,html,'utf8')}}catch(error){console.error('SQ AI Tools UI injection failed:',error?.message||error)}

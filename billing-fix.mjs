...
function verifyPaddleSignature(rawBody, signature, secret) {
  ...
  try{return crypto.timingSafeEqual(Buffer.from(expected,'hex'),Buffer.from(h1,'hex'));}catch{return false;}
}
...
async function createCheckout(req,res){
  ...
  try{
    ...
    console.error('paddle_transaction_network_failed',error?.message || error);
    return res.status(503).json({error:'paddle_unavailable'});
  }
}
  ...
  try{
    const processEvent=billingDb.transaction(()=>{
      ...
      if(eventType==='subscription.activated'){
        ...
        if(userId)billingDb.prepare("UPDATE users SET billing_status='active' WHERE id=?").run(userId);
      }
      ...
      if(eventType==='transaction.past_due' || eventType==='transaction.payment_failed'){
        const userId=Number(data?.custom_data?.sq_ai_user_id || 0);
        if(userId)billingDb.prepare("UPDATE users SET billing_status='past_due' WHERE id=?").run(userId);
      }

...
// Capture the exact request bytes before express.json parses the body. Paddle
// signatures are calculated over ts:rawBody, so the raw payload must survive.
...
async function createCheckout(req, res) {
  ...
  const body = {
    ...
    custom_data: {
      sq_ai_user_id: String(req.user.id),
      sq_ai_plan: requestedPlan
    }
  };
...
async function paddleWebhook(req, res) {
  ...
  const duplicate = billingDb.prepare('SELECT id FROM billing_events WHERE event_id=?').get(eventId);
  if (duplicate) return res.status(200).send('ok');

  try {
    billingDb.prepare('INSERT INTO billing_events(event_id,event_type,transaction_id) VALUES(?,?,?)')
      .run(eventId, eventType, event?.data?.id || null);
    ...
    if (eventType === 'transaction.completed') {
      ...
      if (userId && plan && creditsForPlan[plan]) {
        billingDb.prepare(`UPDATE users SET plan=?, credits=?, paddle_subscription_id=COALESCE(?,paddle_subscription_id), billing_status='active' WHERE id=?`)
          .run(plan, creditsForPlan[plan], data.subscription_id || null, userId);
      }
    }
...
// Replace only SQ AI's old placeholder route. The existing auth middleware is
// preserved, so the new checkout handler still receives req.user.
appPrototype.post = function patchedPost(path, ...handlers) {
  if (path === '/api/billing/checkout' && handlers.length) {
    return originalPost.call(this, path, ...handlers.slice(0, -1), createCheckout);
  }
  return originalPost.call(this, path, ...handlers);
};

// Register Paddle's public webhook endpoint immediately before the server starts.

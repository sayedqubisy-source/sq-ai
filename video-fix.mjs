...
function findFile(value) {
  ...
  if (typeof value === 'string') {
    ...
    if (/\.(mp4|webm|mov)(\?|$)/i.test(value) || /gradio_api\/file=/i.test(value)) return value;
    return null;
  }
...
async function freeVideo(payload) {
  ...
  try {
    // IMPORTANT: the current public Space exposes generate_video with exactly
    // four inputs: input_image, prompt, aspect_ratio, duration.
    // Do not send the internal model parameters; the Space owns those values.
    const body = JSON.stringify({
      data: [null, prompt, aspect, duration]
    });
    ...
    if (!submit.ok) {
      throw Object.assign(
        new Error(data?.error || `Free video service rejected the request (${submit.status}).`),
    ...
    const result = await requestWithRetry(
      `${base}/gradio_api/call/generate_video/${encodeURIComponent(data.event_id)}`,
    ...
    if (!result.ok) {
      throw Object.assign(new Error(`Free video result request failed (${result.status}).`), { upstreamStatus: result.status });
    }
    ...
    if (!video?.ok) {
      throw Object.assign(new Error(`Generated video download failed (${lastStatus}).`), { upstreamStatus: lastStatus });
    }
    ...
    if (head.includes('<!doctype html') || head.includes('<html')) {
      throw new Error('Generated video download returned an HTML error page.');
    }
    if (contentType.includes('json') || contentType.includes('text/html')) {
      throw new Error('Generated video download returned an invalid response.');
    }
    ...
    if (error?.name === 'AbortError') {
      return new Response(JSON.stringify({
        error: {
          code: 'free_video_timeout',
          message: 'Video generation timed out. The free GPU queue may be busy; please try again.'
        }
    ...
    return new Response(JSON.stringify({
      error: {
        code: status === 502 ? 'free_video_bad_gateway' : 'free_video_unavailable',
        message: error?.message || 'Free video service is temporarily unavailable.'
      }
